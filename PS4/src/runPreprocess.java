...
public class runPreprocess {
	...
	public void testing(Bayes bayes, Preprocessing p) throws Exception {
		...
		BufferedReader br = new BufferedReader(new FileReader("positiveTest"));
		String rec = "";
		while((rec = br.readLine())!=null) {
			String[] words = rec.split(" ");
			double[] probs = bayes.calcProbs(words);
			String answer = bayes.sentiment(probs);
			System.out.printf("%s | Sentiment: %s Positive: %.2f Negative: %.2f Neutral: %.2f\n",rec,answer,probs[0],probs[1],probs[2]);
		}
		br.close();
		System.out.println("NEGATIVE-------------");
		BufferedReader br2 = new BufferedReader(new FileReader("negativeTest"));
		...
		BufferedReader br3 = new BufferedReader(new FileReader("neutralTest"));
